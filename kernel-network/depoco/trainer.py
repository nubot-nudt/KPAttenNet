...
class DepocoNetTrainer():
    ...
    def train(self, verbose=True):
        ...
        ###要在这里面添加两行代码，把self.decoder和self.encoder部分放在dataparaell
    ...
    def encode(self,input_dict,float_16=False):
        map_idx = input_dict['idx']
        # print('map:', map_idx)
        scale = input_dict['scale']
        input_dict['features'] = input_dict['features'].to(self.device)
        ...
        ####### Cast to float_16 if necessary #######
        out_dict = self.encoder_model(input_dict.copy())
        out_point_nd = out_dict['points']
        out_map_nd = out_dict['map']
        point_in_map = np.isin(out_point_nd.detach().cpu().numpy(),out_map_nd.detach().cpu().numpy())
        if float_16:
            out_dict['points'] = out_dict['points'].half().float()
            out_dict['features'] = out_dict['features'].half().float()
        nr_emb = out_dict['points'].shape
        comprepoint = out_dict['points']
        #compre_in_map = np.isin(comprepoint.detach().cpu().numpy(),out_map_nd.detach().cpu().numpy())
        return comprepoint,nr_emb,out_dict
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default='config/kitti_decoder/e0_decoder.yaml',
