...
class SubMapParser():
    def __init__(self, config):
        ...
        print(f"train_folder有{self.train_folders}")
        print(f"test_folder有{self.test_folders}")
        print(f"valid_folder有{self.valid_folders}")
        print(f"cols有{cols}")
        # Trainingset
...
class SubMapDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.cols <= 3:
            ...
            if self.init_ones:###这里面np.ones也由float32改成了64
    ...
    def __len__(self):
        #print(f"submap的长度是{self.submaps}")
...
####这里面因为oxford应该是已经经过了归一化的,所以将其改为True,后面还能改回去
    ...
    ###这里面用oxford——robocar的数据集，暂时将normalize改为false
    def getPoints(self, normalize=False):
        # points = pcu.loadCloudFromBinary(self.file)
...
def createSubmaps(folders, nr_submaps=0, cols=3, on_the_fly=False,grid_size=40):
    ...
    #print(f"submap_files有",submap_files)
    if int(nr_submaps) != 0:
