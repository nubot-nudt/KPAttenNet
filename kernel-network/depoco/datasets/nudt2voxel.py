...
def parse_poses_withoutcalib(filename):
...
class Kitti2voxelConverter():
    def __init__(self, config):
        ...
        print(f"test_folders是",self.test_folders)
    ...
    def sparsifieO3d(self, poses, key_pose_idx, seq_path, distance_matrix):
        ...
        for i in valid_scans:
            ...
            poses_withoutvota = poses[i]
            poses_withoutvota[0:3,0:3] = np.eye(3)
            points=np.matmul(poses_withoutvota, scan_hom[valid_p, :].T).T
            # points = scan_hom[valid_p, :]
            #### intensity and label #######
            ...
            # ppp=o3d.geometry.PointCloud()
            # cloud_ppp=np.concatenate(point_cld)
            # ppp.points=o3d.utility.Vector3dVector(cloud_ppp)
    ...
    def convert(self):
        ...
        for j, p in enumerate(folders):
            ...
            if not os.path.exists(out_dir):
                os.makedirs(out_dir)
            # calibration=parse_calibration(p + "calib.txt")
            poses=parse_poses_withoutcalib(p+"poses.txt")
            scan_files=[
                f for f in sorted(os.listdir(os.path.join(p, "velodyne")))
                if f.endswith(".bin")]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default="/home/yy/campus_nudt/campus_kitti",
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default='/home/yy/deep-point-map-compression-fuxian/deep-point-map-compression/depoco/config/arch/sample_net_nudt.yaml',
    ...
    input_folder=FLAGS.dataset + '/01/'
    # calibration=parse_calibration(os.path.join(input_folder, "calib.txt"))
    poses=parse_poses_withoutcalib(os.path.join(input_folder, "test.txt"))
    ...
    # 我好像有点想起来了，但凡是出现这种螺旋状的点云，那估计聚合submap或则reshape的时候弄反了，本来1，2，3，4他可能竖着聚合了
    converter.convert()
