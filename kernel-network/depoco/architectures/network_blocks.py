...
import sys
sys.path.append(sys.path.append('/home/yy/deep-point-map-compression-fuxian/deep-point-map-compression'))
...
class MultiheadSA_Layer(nn.Module):
    ...
    def __init__(self, channels, num_heads):
        ...
        # 定义子模块
        self.q_linear = nn.Conv1d(channels, channels,1, bias=False)
        self.k_linear = nn.Conv1d(channels, channels,1, bias=False)
        self.v_linear = nn.Conv1d(channels, channels,1, bias=False)
        self.trans_linear = nn.Conv1d(channels, channels,1)
    ...
    def forward(self, x):
        ...
        # 投影到多个子空间
        q = self.q_linear(x).view(batch_size, self.num_heads, self.head_dim, n_points).transpose(1, 2) # b, h, d, n 
        k = self.k_linear(x).view(batch_size, self.num_heads, self.head_dim, n_points).transpose(1, 2) # b, h, d, n
        v = self.v_linear(x).view(batch_size, self.num_heads, self.head_dim, n_points).transpose(1, 2) # b, h, d, n
        
        # 计算注意力权重
        scores = torch.matmul(q.transpose(-2, -1), k) / math.sqrt(self.head_dim) # b, h, n, n
        attn = F.softmax(scores, dim=-1)
        attn = attn / (1e-9 + attn.sum(dim=1, keepdims=True))
        # 加权平均得到新的表示
        x_r = torch.matmul(attn, v.transpose(-2, -1)) # b, h, d, n
        x_r = x_r.transpose(1, 2).contiguous().view(batch_size, -1, n_points) # b, c, n
        
        # 残差连接和归一化操作
        
        x_r = self.act(self.after_norm(self.trans_linear(x - x_r)))
        x = x+ x_r
        # 返回结果
        return x    
...
class GridSampleConv(nn.Module):
    def __init__(self, config: dict):
        ...
        # self.sa = nn.Identity()
        # self.confuse = nn.Sequential(nn.Linear(out_fdim + 3, out_fdim),
        #                            nn.BatchNorm1d(out_fdim ),
        #                            nn.LeakyReLU())
        if in_fdim > 1:
            pre_blocks = [
    ...
    def forward(self, input_dict: dict) -> dict:
        map_raw = input_dict['map']
        source = input_dict['points']
        source_is_in = np.isin(source.detach().cpu().numpy(),map_raw.detach().cpu().numpy())
        ...
        features = self.kp_conv.forward(q_pts=input_dict['points'][sample_idx, :],
                                        ...
                                        x=features)
        # point_feature = torch.cat([features.unsqueeze(0),input_dict['points'][sample_idx, :].unsqueeze(0)],dim=2)
        # point_feature = self.confuse(point_feature.squeeze(0))
        
        # features = self.sa(point_feature.unsqueeze(0).permute(0,2,1))
...
#         # Compute the attention weights using dot product between Q and K.
#         weights = torch.matmul(Q, K.transpose(-2, -1)) / (K.shape[-1] ** 0.5)
#         attn_weights = nn.functional.softmax(weights, dim=-1)

#         # Apply the attention weights to the value tensor and compute the output.
...
class AdaptiveDeconv(nn.Module):
    def __init__(self, config: dict):
        ...
        trans_blocks = [
                        nn.Linear(config['in_fdim'], out_features=feature_space),
                        nn.LeakyReLU(),
                        nn.Linear(in_features=feature_space,
                                  out_features=3*self.upsampling_rate),
